...
def trainLaplace(i):
    ...
    print(globalNum, i)
    titers = [int(30e5), int(30e5), int(50e6), int(50e6)]  # Q-learning iterations
    ...
    laplaceModel.plotSingleEigen(
        int(i/2), "images/policies" + str(globalNum) + "/eigen" + str(i) + ".png")

    Qlearner = SimpleQLearner(env, rew, laplaceModel.keyInd)
    Qlearner.train(titers[globalNum-1])
    Qlearner.plotPolicy("images/policies" + str(globalNum) + "/eigenpolicy" + str(i) + ".png")
    Qlearner.savePolicy("data/dat" + str(globalNum) + "/policies/eigenpolicy" + str(i) + ".csv")
